...
type EncodedTerm<T extends IncompleteTerm> = T extends object
    ? { encoding: string } & {
...
const encode = <T extends IncompleteTerm>(term: T, encoding: string = '0'): EncodedTerm<T> => {
    switch (term.type) {
        case TermType.Value: {
...
export type { Application, Term, IncompleteApplication, IncompleteTerm, Value, EncodedTerm };
export { apply, createValue, lambda, namedApply, namedLambda, TermType, validateTerm, encode };
